...
public class Camada{
	...
	public Camada(int mapaWidth, int mapaHeight, int tileWidth, int tileHeight, String img, String arquivo) {
		...
		try {
			tileSet = ImageIO.read(getClass().getClassLoader().getResourceAsStream(img));
		} catch (IOException e) {
	...
	public void montarMapa(int lar, int alt) {
		...
		for (int i = 0; i < mapaWidth; i++) {
			for (int j = 0; j < mapaHeight; j++) {
				...
				camada.getGraphics().drawImage(tileSet, (j * tileHeight), (i * tileWidth),
						(j * tileHeight) + tileHeight, (i * tileWidth) + tileWidth, (tileCol * tileHeight),
						(tileRow * tileWidth), (tileCol * tileHeight) + tileHeight, (tileRow * tileWidth) + tileWidth,
						null);
				if (mapa[i][j] != 0) {
					collision.add(new Rectangle((j * tileWidth),(i * tileHeight), tileWidth, tileHeight));
				}
			}
		}
